...
command_line_args = ["eventgenerator.py", "-e", "TestNamespace", "Test", "TestEvent", "This is a test description", "public static event Action TestEvent;", "doesn't apply to anything",
                     "-fv", "`Action`",
                     "-ex",
...
def start(cl_args: list[str] = sys.argv):
    try:
        args = [ArgWrapper("-e", ["namespace", "class", "name", "description", "declaration", "applies_to"]),
                ArgWrapper("-fv", ["type"]), 
...
def create_event_page(args: ArgParser):
    ...
    full_path = path.join(convert_to_api_reference_path(namespace, class_, "events"), name.lower() + ".md")

    data = {"names": [name], "descriptions": [description], "links": [f"{namespace.lower()}/{class_.lower()}/events/{name.lower()}"]}
    ...
    SidebarManager.add_event(namespace, class_, name)
